...
class VetSchema(Schema):
    ...
    name = fields.Str(validate=lambda str: str.startswith("Dr."))  # validates that the name starts with "Dr."
    email = fields.Email()   #Email has built-in validation
    website = fields.URL()   #URL has built-in validation
    specialty = fields.Str(validate=validate.Length(min=1))  # validates that the string is not empty
    years_practice = fields.Int(validate=validate.Range(min=0, max=100))  # validates that the number is non-negative
    diploma = fields.Str(validate=validate.OneOf(["DVM", "VMD"])) #these are vet degrees, Doctor of Veterinary Medicine and Veterinary Medical Doctor
